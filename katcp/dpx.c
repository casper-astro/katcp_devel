    ...
    if(rh && rh->r_reply && rh->r_message){
      if(strcmp(rh->r_message, str + 1)){
        ...
        if(type == KATCP_REPLY){
          ...
          rh->r_reply = NULL;
          /* TODO: release endpoint here too */
        }
      }
    } else {
      if(type == KATCP_REPLY){
...
int wake_endpoint_peer_flat_katcp(struct katcp_dispatch *d, struct katcp_endpoint *ep, struct katcp_message *msg, void *data)
{
  ...
  switch(type){
    ...
    case KATCP_REPLY   :
      ...
      if(fx->f_replies[0].r_reply){
        fx->f_current_endpoint = fx->f_replies[0].r_issuer;
        rh = &(fx->f_replies[0]);
      } else {
...
int is_inner_flat_katcp(struct katcp_dispatch *d)
{
  ...
  switch(fx->f_current_endpoint == fx->f_remote){
    case KATCP_DIRECTION_REMOTE :
      return 0;
    case KATCP_DIRECTION_INNER :
      return 1;
