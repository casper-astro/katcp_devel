...
struct katcp_actor *create_actor_type_katcp(struct katcp_dispatch *d, char *str, struct katcp_job *j, struct katcp_notice *n, void *data, char *datatype)
{
  ...
  if (a->a_data != NULL && datatype != NULL){
    a->a_data = data;
    a->a_data_type = find_name_type_katcp(d, datatype);
  }
...
int pushstack_statemachine_kcs(struct katcp_dispatch *d, struct katcp_stack *stack, struct katcp_stack_obj *o)
{
#if 0
  struct kcs_sm *m;
...
#define ARG_BASE 5
  ...
  o = create_obj_stack_katcp(temp, t, 1);
  if (o == NULL){
    (*t->t_free)(temp);
#ifdef DEBUG
...
int statemachine_run_ops_kcs(struct katcp_dispatch *d, struct katcp_notice *n, struct kcs_sched_task *t)
{
  ...
  for (i=0; i<s->s_op_list_count; i++){
    op = s->s_op_list[i];
    if (op != NULL){
      rtn = (*(op->o_call))(d, stack, op->o_stack_obj);
#ifdef DEBUG
...
int statemachine_run_kcs(struct katcp_dispatch *d)
{
...
#ifdef DEBUG
  fprintf(stderr,"-----------------------\n");
  fprintf(stderr, "statemachine RUN: PC is %p\n", m->m_pc);
#endif
  ...
  do {
    ...
    for (i=0; i<s->s_edge_list_count; i++){
      ...
      if (e != NULL){
        ...
        if (rtn == 0){
          m->m_pc = e->e_next;
#ifdef DEBUG
          fprintf(stderr, "statemachine RUN: EDGE RETURNS SUCCESS\n");
#endif
          break;
        } 
#ifdef DEBUG
    ...
    if ((s == m->m_pc) && (i == s->s_edge_list_count)){
#ifdef DEBUG
      fprintf(stderr, "statemachine RUN: ALL EDGES FAILED ENDING\n");
#endif
      run = 0;
    }
