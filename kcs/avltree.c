  ...
  if (c == NULL){
    ...
    switch (npdir){
      case AVL_LEFT:
        np->n_balance++;
#if DEBUG >1
        fprintf(stderr,"avl_tree:\t-- balance in %s %d\n", np->n_key, np->n_balance);
#endif
        break;
      case AVL_RIGHT:
        np->n_balance--;
...
int del_node_avltree(struct avl_tree *t, struct avl_node *n)
{
  ...
  /*if (c->n_balance == 0){
#if DEBUG > 1
    fprintf(stderr,"avl_tree:\t0 balance at %s STOP AND GO TO ROOT\n", c->n_key);   
#endif
    flag = 1;
  }*/
  while (run){
