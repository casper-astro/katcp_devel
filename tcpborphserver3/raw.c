...
int setup_raw_tbs(struct katcp_dispatch *d, char *bofdir, int argc, char **argv)
{
  ...
  result += register_flag_mode_katcp(d, "?tap-arp-config",    "set several arp parameters (?tap-arp-config register-name [mode|valid-timeout|query-start|query-stop|query-step|announce-start|announce-stop|announce-step])", &tap_config_cmd, 0, TBS_MODE_RAW);
